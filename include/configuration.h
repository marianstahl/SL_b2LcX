...
public:
  ...
  configuration(const TString jobname){
    ...
    else if(particle.Contains("Lc")){XcMlo = 2210.0; XcMhi = 2368.0;}
  ...
  void environment(TString machine){
    ...
    if(machine.Contains("d0new") || machine.Contains("sigma0") || machine.Contains("raid") || machine.Contains("lambdac")){
      lxplus = false;
  ...
  void set_particle(const TString part){
    if(!(part.Contains("Xic") || part.Contains("Omegac") || part.Contains("Lc")))cout << "sure about the particle?" << endl;
      particle = part;
      if(particle.Contains("Omegac")){XcMlo = 2616.0; XcMhi = 2774.0;}
      else if(particle.Contains("Lc")){XcMlo = 2210.0; XcMhi = 2360.0;}
      else {XcMlo = 2388.0; XcMhi = 2546.0;}
      return;
  }
